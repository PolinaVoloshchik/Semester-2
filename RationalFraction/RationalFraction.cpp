...
RationalFraction & RationalFraction::operator*(const RationalFraction & fr) const
{
	int num = GetNumerator() * fr.GetNumerator();
	int den = GetDenominator() * fr.GetDenominator();
	RationalFraction result(num, den);
	return result;
}

RationalFraction & RationalFraction::operator/(const RationalFraction & fr) const
{
	int num = GetNumerator() * fr.GetDenominator();
	int den = GetDenominator() * fr.GetNumerator();
	RationalFraction result(num, den);
	return result;
}
